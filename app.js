...
app.post('/', function (req, res) {
  ...
  const paramReceived = req.body.result.action;
  if (paramReceived.includes("music")) {
    var type = req.body.result.parameters.music;
    if (type === "music") {
      type = "rock";
    }
    request({
    ...
    var x = req.body.result.parameters.places.toLowerCase();
    console.log(x);
    if(x === "places" || x === "places") {
      x = "cafe";
    }
    request({
      url: 'https://maps.googleapis.com/maps/api/place/nearbysearch/json',
